# this file is a close implementation of the initial algortihm use in:
# Evolution of oil droplets in a chemorobotic platform,
# J. M. P. Gutierrez, T. Hinkley, J. W. Taylor, K. Yanev, L. Cronin,
# Nat. Commun. , 2014 , 5 , 5571
#
# The difference is in the use of the roulette function instead of the new, more robust, softmax
# We recommend to not use it as the newer version have better properties
...
def roulette(values):
    # compared to the proper GA, this function has two problems
    # 1) it assumed only positive fitnesses.. thus if negative fitness values we pull them up to zero
    # 2) it really depends on the abasolute value on the fitness, if all fitness are very high they will mostly all be equality probable.
    # fortunatly in the first experiment fitness where always positive and always relatively small (<10), so it was working fine
    # this is why the new implementation use the softmax function that eliminate all such problems
    # as you see this function does not much, it was only doing the probabilistic_choice on the raw fitnesses value
    # we added the softmax so the fitnesses are first transformed in a smart way before doing the probailistic sampling
    # this roulette function replace the softmax function but simply does not affect the fintesses except than avoiding negative values which would make bad things happen in term of probability
    x = numpy.clip(values, 0, numpy.max(values))
    return proba_normalize(x)
...
class GA_DROPBOT1(object):
    ...
    def draw_survivors_id(self):
        # survivors are sample probabilisticaly wrt. their fitness
        fitnesses = self.get_fitnesses()
        x = roulette(fitnesses)
